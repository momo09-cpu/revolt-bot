...
`.trim();
  ...
  try {
    ...
    const reply = await conn.reply(m.chat, '> *Personaggio aggiunto con successo!*\n\nRispondi a questo messaggio con "invia" o "👍" solo se desideri inviare i personaggi ai miei creatori per aggiungerli a *revolt-bot*.', m);
    ...
    handler.before = async function (m, { conn }) {
      if (m.quoted && m.quoted.id === reply.id && ['invia', '👍'].includes(m.text.toLowerCase())) {
        ...
        await conn.reply('393515533859@s.whatsapp.net', `*Richiesta di @${m.sender.split("@")[0]} per aggiungere personaggi di Fantasy RPG in revolt-bot*`, null, { mentions: [m.sender] });
        ...
        await conn.reply(m.chat, `File inviato ai miei creatori! Continua ad aggiungere altri personaggi che desideri vedere in revolt-bot`, m);
      }
    };
  } catch (error) {
